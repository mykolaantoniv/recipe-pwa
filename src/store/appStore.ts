  ...
  persist(
    (set, get) => ({
      ...
      addToShoppingList: (items) => {
        // Always read fresh state via get() to avoid stale closures
        const current = get().shoppingList;
        const merged = [...current];
        for (const item of items) {
          const key = item.name.toLowerCase().trim();
          const idx = merged.findIndex(i => i.name.toLowerCase().trim() === key);
          if (idx !== -1) {
            const existAmt = parseFloat(merged[idx].amount);
            const newAmt = parseFloat(item.amount);
            const unit = item.amount.replace(/^[\d.,]+\s*/, '').trim();
            merged[idx] = {
              ...merged[idx],
              amount: (!isNaN(existAmt) && !isNaN(newAmt))
                ? `${existAmt + newAmt} ${unit}`.trim()
                : merged[idx].amount,
            };
          } else {
            merged.push(item);
